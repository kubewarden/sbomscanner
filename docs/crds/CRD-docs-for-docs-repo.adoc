...
[id="{anchor_prefix}-github-com-kubewarden-sbomscanner-api-storage-v1alpha1-epss"]
==== EPSS
...
|===
| Field | Description | Default | Validation
| *`score`* __string__ | Score is the probability (0..1) that the CVE will be exploited +
in the next 30 days + |  | 
| *`percentile`* __string__ | Percentile is the rank of the score among all scored CVEs (0..1) + |  | 
| *`date`* __link:https://kubernetes.io/docs/reference/generated/kubernetes-api/v1.36/#time-v1-meta[$$Time$$]__ | Date is the score_date of the EPSS feed, when the scores were computed + |  | 
|===
...
[id="{anchor_prefix}-github-com-kubewarden-sbomscanner-api-storage-v1alpha1-kev"]
==== KEV
...
|===
| Field | Description | Default | Validation
| *`dateAdded`* __string__ | DateAdded is the date CISA added the CVE to the catalog, as published (YYYY-MM-DD) + |  | 
| *`dueDate`* __string__ | DueDate is the remediation due date set by CISA, as published (YYYY-MM-DD) + |  | 
| *`knownRansomwareCampaignUse`* __xref:{anchor_prefix}-github-com-kubewarden-sbomscanner-api-storage-v1alpha1-ransomwarecampaignuse[$$RansomwareCampaignUse$$]__ | KnownRansomwareCampaignUse is the CISA assessment of use in ransomware campaigns + |  | 
|===
...
[id="{anchor_prefix}-github-com-kubewarden-sbomscanner-api-storage-v1alpha1-ransomwarecampaignuse"]
...
.Appears In:
****
- xref:{anchor_prefix}-github-com-kubewarden-sbomscanner-api-storage-v1alpha1-kev[$$KEV$$]
****
...
| *`kev`* __xref:{anchor_prefix}-github-com-kubewarden-sbomscanner-api-storage-v1alpha1-kev[$$KEV$$]__ | KEV is the CISA Known Exploited Vulnerabilities catalog entry for the CVE. +
Nil when the CVE is not in the catalog or no KEV data is available. + |  | 
| *`epss`* __xref:{anchor_prefix}-github-com-kubewarden-sbomscanner-api-storage-v1alpha1-epss[$$EPSS$$]__ | EPSS is the Exploit Prediction Scoring System data for the CVE. +
Nil when no EPSS data is available. + |  | 
|===
