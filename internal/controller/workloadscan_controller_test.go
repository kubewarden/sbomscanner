							...
							MatchConditions: []v1alpha1.MatchCondition{
								{
									Name:       "tag-v1",
									Expression: `tag == "v1"`,
									Labels:     map[string]string{namespace.Name: "true"},
								},
							},
						},
					},
							...
							MatchConditions: []v1alpha1.MatchCondition{
								{
									...
									Labels:     map[string]string{otherNamespace.Name: "true"},
								},
							},
						},
					},
	...
	When("WorkloadScanConfiguration exists", func() {
		...
		When("namespace matches selector and has pods", func() {
			...
			It("should create Registry and WorkloadScanReport for a standalone pod", func(ctx context.Context) {
				...
				Expect(registry.Spec.Repositories[0].MatchConditions[0].Labels).To(HaveKeyWithValue(namespace.Name, "true"))
		...
		When("TargetNamespace is configured", func() {
			...
			BeforeEach(func(ctx context.Context) {
				...
				configuration.Spec.TargetNamespace = targetNamespace.Name
			...
			It("should create registries in the target namespace", func(ctx context.Context) {
				...
				Expect(report.Spec.Containers[0].ImageRef.Namespace).To(Equal(targetNamespace.Name))
			})
			...
			It("should merge conditions from multiple namespaces using labels", func(ctx context.Context) {
				...
				Expect(conditionsByName["tag-v1"].Labels).To(HaveKeyWithValue(sourceNamespaceA.Name, "true"))
				Expect(conditionsByName["tag-v1"].Labels).To(HaveKeyWithValue(sourceNamespaceB.Name, "true"))

				By("Verifying tag-v2 has label only for namespace B")
				Expect(conditionsByName).To(HaveKey("tag-v2"))
				Expect(conditionsByName["tag-v2"].Labels).NotTo(HaveKey(sourceNamespaceA.Name))
				Expect(conditionsByName["tag-v2"].Labels).To(HaveKeyWithValue(sourceNamespaceB.Name, "true"))
			})
			...
			It("should remove namespace label when namespace stops matching", func(ctx context.Context) {
				...
				Expect(registry.Spec.Repositories[0].MatchConditions).To(HaveLen(1))
				Expect(registry.Spec.Repositories[0].MatchConditions[0].Labels).To(HaveKeyWithValue(sourceNamespaceA.Name, "true"))
				Expect(registry.Spec.Repositories[0].MatchConditions[0].Labels).To(HaveKeyWithValue(sourceNamespaceB.Name, "true"))
				...
				Expect(registry.Spec.Repositories[0].MatchConditions).To(HaveLen(1))
				Expect(registry.Spec.Repositories[0].MatchConditions[0].Labels).NotTo(HaveKey(sourceNamespaceA.Name))
				Expect(registry.Spec.Repositories[0].MatchConditions[0].Labels).To(HaveKeyWithValue(sourceNamespaceB.Name, "true"))
			})
