...
// ScanNodeSBOMHandler is responsible for handling SBOM scan requests.
type ScanNodeSBOMHandler struct {
...
// NewScanNodeSBOMHandler creates a new instance of ScanNodeSBOMHandler.
...
func (h *ScanNodeSBOMHandler) Handle(ctx context.Context, message messaging.Message) error { //nolint:funlen,gocognit,gocyclo,cyclop // TODO: refactor this function in smaller ones
	...
	h.logger.InfoContext(ctx, "SBOM scan requested",
		"nodesbom", scanNodeSBOMMessage.NodeSBOM.Name,
		"namespace", scanNodeSBOMMessage.NodeSBOM.Namespace,
	)

	scanJob := &v1alpha1.NodeScanJob{}
	err := h.k8sClient.Get(ctx, client.ObjectKey{
		Name:      scanNodeSBOMMessage.NodeScanJob.Name,
		Namespace: scanNodeSBOMMessage.NodeScanJob.Namespace,
	}, scanJob)
	if err != nil {
		if apierrors.IsNotFound(err) {
			// Stop processing if the scanjob is not found, since it might have been deleted.
			h.logger.ErrorContext(ctx, "NodeScanJob not found, stopping SBOM scan", "nodescanjob", scanNodeSBOMMessage.NodeScanJob.Name, "namespace", scanNodeSBOMMessage.NodeScanJob.Namespace)
			return nil
		}
		return fmt.Errorf("failed to get NodeScanJob: %w", err)
	}
	if string(scanJob.GetUID()) != scanNodeSBOMMessage.NodeScanJob.UID {
		h.logger.InfoContext(ctx, "NodeScanJob not found, stopping SBOM generation (UID changed)", "nodescanjob", scanNodeSBOMMessage.NodeScanJob.Name, "namespace", scanNodeSBOMMessage.NodeScanJob.Namespace,
			"uid", scanNodeSBOMMessage.NodeScanJob.UID)
		return nil
	}

	h.logger.DebugContext(ctx, "NodeScanJob found", "nodescanjob", scanJob)

	if scanJob.IsFailed() {
		h.logger.InfoContext(ctx, "NodeScanJob is in failed state, stopping SBOM scan", "nodescanjob", scanJob.Name, "namespace", scanJob.Namespace)
		return nil
	}

	nodeSbom := &storagev1alpha1.NodeSBOM{}
	err = h.k8sClient.Get(ctx, client.ObjectKey{
		Name:      scanNodeSBOMMessage.NodeSBOM.Name,
		Namespace: scanNodeSBOMMessage.NodeSBOM.Namespace,
	}, nodeSbom)
	if err != nil {
		// Stop processing if the SBOM is not found, since it might have been deleted.
		if apierrors.IsNotFound(err) {
			h.logger.ErrorContext(ctx, "SBOM not found, stopping SBOM scan", "sbom", scanNodeSBOMMessage.NodeSBOM.Name, "namespace", scanNodeSBOMMessage.NodeSBOM.Namespace)
			return nil
		}
	...
	_, err = sbomFile.Write(nodeSbom.SPDX.Raw)
	if err != nil {
	...
	defer func() {
		...
		if err = os.Remove(reportFile.Name()); err != nil {
			h.logger.Error("failed to remove temporary report file", "error", err)
		}
	}()
	...
	h.logger.InfoContext(ctx, "node SBOM scanned",
		"sbom", scanNodeSBOMMessage.NodeSBOM.Name,
		"namespace", scanNodeSBOMMessage.NodeSBOM.Namespace,
	)
	...
	nodeVulnerabilityReport := &storagev1alpha1.NodeVulnerabilityReport{
		ObjectMeta: metav1.ObjectMeta{
			Name:      nodeSbom.Name,
			Namespace: nodeSbom.Namespace,
		},
	}
	if err = controllerutil.SetControllerReference(nodeSbom, nodeVulnerabilityReport, h.scheme); err != nil {
	...
	_, err = controllerutil.CreateOrUpdate(ctx, h.k8sClient, nodeVulnerabilityReport, func() error {
		...
		nodeVulnerabilityReport.NodeMetadata = nodeSbom.GetNodeMetadata()
...
func (h *ScanNodeSBOMHandler) setupVEXHubRepositories(vexHubList *v1alpha1.VEXHubList, trivyVEXPath, vexRepoPath string) error {
