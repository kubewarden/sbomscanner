...
CREATE INDEX IF NOT EXISTS idx_workloadscanreports_containers ON workloadscanreports USING GIN ((object->'containers'));
`
...
func NewWorkloadScanReportStore(
	...
	nc *nats.Conn,
	logger *slog.Logger,
) (*RegistryStoreWithWatcher, *WorkloadScanReportWatcher, error) {
	...
	derivedWatcher := newWorkloadScanReportDerivedWatcher(
		nc,
		db,
		workloadScanReportResourcePluralName,
		natsBroadcaster,
		store,
		logger,
	)
