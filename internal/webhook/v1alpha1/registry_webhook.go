...
func (v *RegistryCustomValidator) ValidateUpdate(ctx context.Context, _, registry *v1alpha1.Registry) (admission.Warnings, error) {
...
func (v *RegistryCustomValidator) validatedManagedRegistry(ctx context.Context, registry *v1alpha1.Registry) error {
	...
	if err != nil {
		return apierrors.NewInternalError(fmt.Errorf("unable to retrieve admission request from context"))
	}
	...
	if request.UserInfo.Username != allowedUsername {
		return apierrors.NewForbidden(
			...
			registry.Name,
			fmt.Errorf("only the designated service account can modify managed Registry resources"),
		)
	}
